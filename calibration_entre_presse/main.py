...
if __name__ == "__main__":
    ...
    objp[:, :2] = np.stack(np.meshgrid(np.arange(COL), np.arange(ROW)), axis=-1).reshape(-1, 2).astype(np.float32)
    objp *= square_size
    ...
    with open(f"{out_calib_name}_{date_str}.pkl", "wb") as f:
        pickle.dump({
            "ret": ret,
            "K": K,
            "dist": dist,
    ...
    for i in range(len(imgpoints)):
        if len(imgpoints[i]) != 0:
            pts_world_all.append(pixels_to_world(imgpoints[i], K, dist, rvecs, tvecs, i))
    ...
    keep_idx = keep_first_in_radius(imgpoints, r=5.0)
    ...
    # MIL calibration
    app = MIL.MappAlloc(MIL.M_DEFAULT, MIL.M_NULL)
    sys = MIL.MsysAlloc(app, MIL.M_SYSTEM_HOST, MIL.M_DEFAULT, MIL.M_DEFAULT)
    display = MIL.MdispAlloc(sys, MIL.M_DEFAULT, "M_DEFAULT", MIL.M_WINDOWED, MIL.M_NULL)
    ...
    MIL.McalSave(f"{out_calib_name}_{date_str}.mca", cal_ctx, MIL.M_DEFAULT)
    ...
    src_img = MIL.MbufRestore(images[0], sys, MIL.M_NULL)
